...
`docs/records/beyond_market_shadow_reopening.json`. Grava somente em
`data/market_shadow.jsonl`; a produção `data/market.db` permanece sob o
encerramento original e não é tocada por este script.
"""
...
def enrich_with_frozen_model(quote: dict, team_a: str, team_b: str) -> dict:
    prediction = predict_match(
        team_a,
        team_b,
        fmt=quote.get("format") or "bo3",
        scheduled_start_at=(
            datetime.fromisoformat(quote["scheduled_at"].replace("Z", "+00:00"))
            if quote.get("scheduled_at")
            else None
        ),
        dry_run=True,
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--event-id", required=True, help="ID Gamma explícito; evita casamento ambíguo"
    )
    parser.add_argument("--output", type=Path, default=ROOT / "data" / "market_shadow.jsonl")
    args = parser.parse_args(argv)
    quote = PolymarketProvider().fetch_match(args.team_a, args.team_b, event_id=args.event_id)
    quote = enrich_with_frozen_model(quote, args.team_a, args.team_b)
    inserted = append_once(args.output, quote)
    print(
        json.dumps(
            {"inserted": inserted, "quote": quote}, ensure_ascii=False, indent=2, sort_keys=True
        )
    )
    return 0
