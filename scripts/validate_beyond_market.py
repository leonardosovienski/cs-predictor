...
Gate de reabertura shadow-only (não financeiro). Nunca autoriza capital:
`financial_capital_authorized` é sempre `false` na saída.
"""
...
def main(argv=None) -> int:
    ...
    try:
        assert_market_shadow_collection_open()
        result = beyond_market_validate(
            _rows(args.input), train_end_at=args.train_end, minimum_test_rows=args.minimum_test_rows
        )
