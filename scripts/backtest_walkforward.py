...
def _p_series(elo_a, elo_b, fmt):
    ...
    return sum(pr for placar, pr in dist.items()
...
def run(cfg, conn):
    ...
    cut = (datetime.fromisoformat(rows[0][1])
           + timedelta(days=int(bt["burnin_days"]))).strftime("%Y-%m-%d")
    ...
    return {"probs_m": probs_m, "probs_b": probs_b, "outs": outs,
            "loss_m": loss_m, "loss_b": loss_b, "elo": elo,
            "n_total": len(rows)}
