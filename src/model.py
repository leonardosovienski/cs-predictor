...
def infer_format(result_a: int, result_b: int, advertised: str | None = None) -> str:
    ...
    if result_a == result_b:
        ...
        raise ValueError(f"placar {result_a}-{result_b} sem vencedor (BO2/empate não suportado)")
...
def update_series_pair(
    ...
    *,
    ...
    """Atualiza uma série via core sem mover sua semântica para fora do domínio."""
    book = RatingBook()
    book.seed(team_a, elo_a)
    book.seed(team_b, elo_b)
    updated_a, updated_b = book.record_match(
        team_a, team_b, score_a=score_a, expected_a=expected_a, k=k
    )
    return updated_a.rating, updated_b.rating
...
def series_win_prob(score_probs: dict) -> float:
    ...
    return sum(pr for placar, pr in score_probs.items() if _wins(placar)[0] > _wins(placar)[1])
...
def cover_probability(
    score_probs: dict, handicap: float, *, side_a: bool = True
) -> tuple[float, float]:
    ...
    for placar, pr in score_probs.items():
        wa, wb = _wins(placar)
        wins, losses = (wa, wb) if side_a else (wb, wa)
        margin = wins + handicap - losses
        if margin > 0:
            covered += pr
        elif margin == 0:
            push += pr
...
def calibrate_score_probs(score_probs: dict, prob_raw: float, prob_cal: float) -> dict:
    ...
    return {
        placar: pr * (scale_a if _wins(placar)[0] > _wins(placar)[1] else scale_b)
        for placar, pr in score_probs.items()
    }
...
class EloModel:
    ...
    def __init__(self, ratings_file: Path | str | None = None):
        ...
        self.path = (
            Path(ratings_file)
            if ratings_file
            else (ROOT / cfg.get("ratings_file", "data/ratings.json"))
        )
        if self.path.exists():
        ...
        self.platt = PlattCalibrator.load(ROOT / "data" / "calibration_platt.json")
    ...
    def _elo(self, name: str) -> tuple[str, float]:
        ...
        exact_ci = [official for official in self.ratings if identity_key(official) == low]
        if len(exact_ci) == 1:
        ...
        hits = [official for official in self.ratings if low in identity_key(official)]
        if len(hits) == 1:
            return hits[0], self.ratings[hits[0]]
        raise ValueError(
            f"time desconhecido: {name!r}" + (f" — você quis dizer {hits}?" if hits else "")
        )
    ...
    def predict_match(self, team_a: str, team_b: str, format: str = "bo3") -> dict:
        ...
        dist = calibrate_score_probs(raw, prob_a, prob_cal) if self.platt else raw
        out = {
            "team_a": a,
            "team_b": b,
            "format": fmt,
    ...
    def predict_handicap(
        self, team_a: str, team_b: str, handicap: float, format: str = "bo3"
    ) -> dict:
    ...
    def update_ratings(
        self, team_a: str, team_b: str, result_a: int, result_b: int, format: str | None = None
    ) -> dict:
        ...
        new_a, new_b = update_series_pair(a, b, elo_a, elo_b, score_a=s_a, expected_a=e_a, k=k)
        delta = new_a - elo_a
        self.ratings[a] = new_a
        self.ratings[b] = new_b
