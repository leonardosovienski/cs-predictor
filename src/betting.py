...
def go_gate(path=None):
    ...
    ready=(d.get('verdict')=='GATE_PASSED_FOR_PROSPECTIVE_SHADOW' and d.get('matured_matches',0)>=d.get('required_matured_matches',1000) and d.get('calendar_days',0)>=d.get('required_calendar_days',90))
    return {'decision':'GATE_PASSED_FOR_PROSPECTIVE_SHADOW' if ready else 'NO-GO',
            'reason':'somente shadow prospectivo autorizado; capital continua bloqueado' if ready else 'amostra financeira/gate insuficiente'}
def record_bet(*,selection,prob_model,decimal_odds,bankroll,real=False,event_id=None,path=None,gate_path=None,**metadata):
    if real: raise PermissionError('aposta real bloqueada: nenhum gate habilita capital automaticamente')
