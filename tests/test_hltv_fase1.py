...
from src import db
from src.data.hltv_provider import (
...
def test_fetch_results_page_rejeita_desafio_200_sem_resultados(monkeypatch):
    ...
    monkeypatch.setattr(
        provider.session, "get", lambda *args, **kwargs: _Response("<html>challenge</html>")
    )
...
def test_fetch_results_page_rejeita_quebra_total_do_parser(monkeypatch):
    ...
    monkeypatch.setattr(provider.session, "get", lambda *args, **kwargs: _Response(html))
...
def test_db_upsert_match_maps_idempotente():
    conn = db.connect(":memory:")
    maps = [
        {"map_name": "Mirage", "team_a": "9z", "team_b": "PARIVISION", "score_a": 13, "score_b": 9}
    ]
...
def test_match_ids_missing_maps(tmp_path):
    ...
    db.upsert_match_maps(
        conn, 1, [{"map_name": "Mirage", "team_a": "A", "team_b": "B", "score_a": 13, "score_b": 9}]
    )
